...
WORKDIR="${FSCA_WORKDIR:-${ROOT_DIR}/fsca-demo-sei-testnet}"

if ! command -v fsca >/dev/null 2>&1; then
  echo "ERROR: 'fsca' command not found. Please run npm link first."
  exit 1
fi
...
if [ -z "${FSCA_PRIVATE_KEY:-}" ] || [ -z "${FSCA_ACCOUNT_ADDRESS:-}" ]; then
  ...
  echo "Please export:"
  echo "  FSCA_PRIVATE_KEY=<your-sei-testnet-private-key>"
  echo "  FSCA_ACCOUNT_ADDRESS=<your-sei-testnet-address>"
  exit 1
fi
...
print_banner "FSCA CBS Demo (Sei Testnet) - START"
...
print_banner "Step 1 - fsca init (Sei testnet params)"
run_cli fsca init \
...
print_banner "Step 2 - fsca cluster init"
run_cli fsca cluster init --threshold "${THRESHOLD}"

print_banner "Step 3 - fsca deploy CBS contracts"
run_cli fsca deploy --contract AccountStorage --description AccountStorage
run_cli fsca deploy --contract TradeEngineV1 --description TradeEngineV1
run_cli fsca deploy --contract RiskGuardV1 --description RiskGuardV1

STORAGE_ADDR="$(node -e "const c=require('./project.json');console.log(c.fsca.alldeployedcontracts.find(x=>x.name==='AccountStorage').address)")"
TRADE_V1_ADDR="$(node -e "const c=require('./project.json');console.log(c.fsca.alldeployedcontracts.find(x=>x.name==='TradeEngineV1').address)")"
RISK_ADDR="$(node -e "const c=require('./project.json');console.log(c.fsca.alldeployedcontracts.find(x=>x.name==='RiskGuardV1').address)")"
...
print_banner "Step 4 - fsca cluster link (before mount)"
run_cli fsca cluster choose "${TRADE_V1_ADDR}"
run_cli fsca cluster link positive "${STORAGE_ADDR}" 1
run_cli fsca cluster link positive "${RISK_ADDR}" 3

print_banner "Step 5 - fsca cluster mount"
run_cli fsca cluster choose "${STORAGE_ADDR}"
run_cli fsca cluster mount 1 AccountStorage

run_cli fsca cluster choose "${TRADE_V1_ADDR}"
run_cli fsca cluster mount 2 TradeEngineV1

run_cli fsca cluster choose "${RISK_ADDR}"
run_cli fsca cluster mount 3 RiskGuardV1

run_cli fsca cluster list mounted
run_cli fsca cluster graph
...
run_cli fsca cluster upgrade --id 2 --contract TradeEngineV2
...
print_banner "FSCA CBS Demo (Sei Testnet) - DONE"
